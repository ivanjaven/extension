...
import { fetchQueue } from '@/server/queries/fetch-queue'
...
export default function DocumentPage() {
  const [activityLog, setActivityLog] = useState<queueTypedef[]>([])
  ...
  useEffect(() => {
    const loadDocumentLogs = async () => {
      ...
      try {
        ...
        const response = await fetchQueue(1, ITEMS_PER_PAGE)
